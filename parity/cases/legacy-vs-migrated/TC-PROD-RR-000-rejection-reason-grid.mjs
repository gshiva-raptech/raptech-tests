// TC-PROD-RR-000 — Admin → Production → Rejection Reason grid — Track B (structure).
export default {
  id: 'TC-PROD-RR-000', title: 'Rejection Reason grid reachable + columns', track: 'B', role: 'regular',
  urlPath: '/admin/production/rejection-reason', module: 'Production', subModule: 'Production → Rejection Reason',
  hints: '- ProductionController rejection-reason grid + rows (custom_status type=Production).',
  async runMigrated({ page, base, creds, forms }) {
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    await page.goto(`${MIG}/admin/production/rejection-reason`, { waitUntil: 'domcontentloaded' });
    ...
    const rows = await page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : null; }, `${MIG}/admin/production/rejection-reason/rows`);
