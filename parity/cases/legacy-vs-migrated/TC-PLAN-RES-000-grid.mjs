// TC-PLAN-RES-000 — Admin → Planning → Resources grid — Track B (structure).
export default {
  id: 'TC-PLAN-RES-000', title: 'Resources grid reachable + columns', track: 'B', role: 'regular',
  urlPath: '/admin/planning/resources', module: 'Admin Settings', subModule: 'Planning → Resources',
  hints: '- PlanningController resources grid + rows.',
  async runMigrated({ page, base, creds, forms }) {
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    await page.goto(`${MIG}/admin/planning/resources`, { waitUntil: 'domcontentloaded' });
    ...
    const rows = await page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : null; }, `${MIG}/admin/planning/resources/rows`);
