// TC-PROD-DR-001 — Admin → Production → Delay Reason create / duplicate / edit / delete — Track B.
// custom_status (type=Task Delay Reason). statusName required; dup-name guard (org+type scoped);
// Grid menu has Edit only; delete endpoint kept (hard delete) used for cleanup.
import { psql } from '../../lib/db.mjs';

export default {
  id: 'TC-PROD-DR-001', title: 'Delay Reason create / duplicate / edit / delete', track: 'B', role: 'regular',
  urlPath: '/admin/production/delay-reason', module: 'Production', subModule: 'Production → Delay Reason',
  hints: '- ProductionController delay-reason create (dup name org+type), {id} update, {id}/delete (hard). custom_status type=Task Delay Reason.',
  data() { return { stamp: Date.now().toString().slice(-7) }; },
  async runMigrated({ page, base, creds, data, forms }) {
    page.on('dialog', d => d.accept().catch(() => {}));
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    const name = `ZZ Delay ${data.stamp}`;
    const rows = async () => page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : []; }, `${MIG}/admin/production/delay-reason/rows`);
    const del = async (id) => page.evaluate(async (a) => { const h = (document.querySelector('meta[name=_csrf_header]') || {}).content; const t = (document.querySelector('meta[name=_csrf]') || {}).content; await fetch(`${a.MIG}/admin/production/delay-reason/${a.id}/delete`, { method: 'POST', headers: h ? { [h]: t } : {}, redirect: 'follow' }); }, { MIG, id });
    const fillNew = async (nm) => { await page.goto(`${MIG}/admin/production/delay-reason/new`, { waitUntil: 'domcontentloaded' }); await page.waitForTimeout(600); await page.fill('#statusName', nm).catch(() => {}); };
    ...
    try {
      ...
      id = (page.url().match(/delay-reason\/(\d+)/) || [])[1] || null;
      ...
      if (id) {
        await page.goto(`${MIG}/admin/production/delay-reason/${id}`, { waitUntil: 'domcontentloaded' });
      ...
      try { psql(`DELETE FROM raptech_scm.custom_status WHERE type='Task Delay Reason' AND status_name LIKE 'ZZ Delay %'`); } catch (e) { /* best-effort */ }
    }
