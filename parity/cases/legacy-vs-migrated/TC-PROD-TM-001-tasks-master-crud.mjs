// TC-PROD-TM-001 — Admin → Production → Tasks Master create / duplicate / edit / details / delete — Track B.
// task_management_tasks. Entity (required, locked on edit) + taskName required; dup-name guard
...
export default {
  id: 'TC-PROD-TM-001', title: 'Tasks Master create / duplicate / edit / details / delete', track: 'B', role: 'regular',
  urlPath: '/admin/production/tasks-master', module: 'Production', subModule: 'Production → Tasks Master',
  hints: '- ProductionController tasks-master create (entity required, dup org+entity), {id} update, {id}/details (read-only), {id}/delete (hard). task_management_tasks.',
  data() { return { stamp: Date.now().toString().slice(-7) }; },
  async runMigrated({ page, base, creds, data, forms }) {
    page.on('dialog', d => d.accept().catch(() => {}));
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    const name = `ZZ Task ${data.stamp}`;
    const rows = async () => page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : []; }, `${MIG}/admin/production/tasks-master/rows`);
    const del = async (id) => page.evaluate(async (a) => { const h = (document.querySelector('meta[name=_csrf_header]') || {}).content; const t = (document.querySelector('meta[name=_csrf]') || {}).content; await fetch(`${a.MIG}/admin/production/tasks-master/${a.id}/delete`, { method: 'POST', headers: h ? { [h]: t } : {}, redirect: 'follow' }); }, { MIG, id });
    ...
    const fillNew = async (nm, ent) => {
      await page.goto(`${MIG}/admin/production/tasks-master/new`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(600);
      const e = ent || await firstEntity();
      if (e) await page.selectOption('#entityId', e).catch(() => {});
      await page.fill('#taskName', nm).catch(() => {});
      await page.fill('#taskDescription', 'desc').catch(() => {});
      return e;
    };
    ...
    try {
      ...
      id = (page.url().match(/tasks-master\/(\d+)/) || [])[1] || null;
      inGrid = id ? (await rows()).some(r => String(r.taskId) === String(id)) : false;
      ...
      if (id) {
        await page.goto(`${MIG}/admin/production/tasks-master/${id}`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(600);
        await page.fill('#taskDescription', `edited ${data.stamp}`).catch(() => {});
        await Promise.all([page.waitForLoadState('networkidle').catch(() => {}), page.getByRole('button', { name: /^update$/i }).click()]);
        await page.waitForTimeout(1000);
        editPersisted = (await rows()).some(r => String(r.taskId) === String(id) && String(r.taskDescription) === `edited ${data.stamp}`);
      }

      if (id) {
        await page.goto(`${MIG}/admin/production/tasks-master/${id}/details`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(600);
      ...
      if (id) { await del(id); await page.waitForTimeout(800); deletedGone = !(await rows()).some(r => String(r.taskId) === String(id)); id = deletedGone ? null : id; }
    } finally {
      if (id) { try { await del(id); } catch (e) { /* best-effort */ } }
      try { psql(`DELETE FROM raptech_scm.task_management_tasks WHERE task_name LIKE 'ZZ Task %'`); } catch (e) { /* best-effort */ }
    }
