// TC-FT-GCF-000 — Form Templates → Global Custom Fields grid — Track B (structure).
export default {
  id: 'TC-FT-GCF-000', title: 'Global Custom Fields grid reachable + columns', track: 'B', role: 'regular',
  urlPath: '/admin/form-templates/global-custom-fields', module: 'Admin Settings', subModule: 'Form Templates → Global Custom Fields',
  hints: '- FormTemplatesController global-custom-fields grid + rows.',
  async runMigrated({ page, base, creds, forms }) {
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    await page.goto(`${MIG}/admin/form-templates/global-custom-fields`, { waitUntil: 'networkidle' });
    ...
    const rows = await page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : null; }, `${MIG}/admin/form-templates/global-custom-fields/rows`);
