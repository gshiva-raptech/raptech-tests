// TC-ITEM-UOM-000 — Admin → Items → UOM grid — Track B (structure).
export default {
  id: 'TC-ITEM-UOM-000', title: 'UOM grid reachable + columns', track: 'B', role: 'regular',
  urlPath: '/admin/items/uom', module: 'Admin Settings', subModule: 'Items → UOM',
  hints: '- ItemsController uom grid + rows.',
  async runMigrated({ page, base, creds, forms }) {
    await forms.loginMigrated(page, base, creds.user, creds.pass);
    const MIG = base.replace(/\/+$/, '');
    await page.goto(`${MIG}/admin/items/uom`, { waitUntil: 'domcontentloaded' });
    ...
    const rows = await page.evaluate(async (u) => { const r = await fetch(u, { headers: { Accept: 'application/json' } }); return r.ok ? await r.json() : null; }, `${MIG}/admin/items/uom/rows`);
